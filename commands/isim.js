...
const shiva = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("shiva").setLabel("İsim Değiştirildi").setStyle(2).setDisabled(true))
 ...
 **\`${member.user.username}\` adlı kullanıcın ismi başarıyla <@${message.author.id}> tarafından \`${Name2} | ${age}\` olarak ayarlandı**
 ...
 const kanal1 = member.guild.channels.cache.find(r => r.id === (config.registerlog)); 
    ...
    message.reply({embeds : [embed],components:[shiva]})
    kanal1.send({ embeds: [shiva2], components: [shiva] })
    }
