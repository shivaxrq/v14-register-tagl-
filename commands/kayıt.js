...
const button1 = new ButtonBuilder().setCustomId('erkek').setLabel("Erkek").setStyle(1).setEmoji("1187707960751833088")
const button2 = new ButtonBuilder().setCustomId('kiz').setLabel("Kadın").setStyle(2).setEmoji("1187707962987384842")
const button4 = new ButtonBuilder().setCustomId('iptal').setLabel("İptal").setStyle(4).setEmoji("1187707956930809917")
const shivabuton = new ButtonBuilder().setCustomId('shiva').setLabel("Kaydı Tamamlandı").setStyle(2).setDisabled(true).setEmoji("1187707955072733184")
const shivabuton2 = new ButtonBuilder().setCustomId('hg').setLabel("Aramıza Hoş Geldin").setStyle(4).setEmoji("🎉").setDisabled(true)
...
.addComponents([shivabuton])
const row3 = new ActionRowBuilder()
.addComponents([shivabuton2])
...
collector.on("collect", async (button) => {
     ...
     if (button.customId === "erkek") {
        ...
        msg.edit({embeds: [shiva2], components: [row2] });
        ...
        kanal1.send({ embeds: [shiva2], components: [row2] })
        }
     }
     ...
     if (button.customId === "kiz") {
        ...
        member.setNickname(`${name} | ${age}`)
        member.roles.remove(ayar.kayıtsız)
        msg.edit({embeds:[shiva], components: [row2] });
          if(ayar.chat && client.channels.cache.has(ayar.chat)) client.channels.cache.get(ayar.chat).send({ content:`Aramıza hoşgeldin **${uye}**! Kuralları okumayı unutma!`, components: [row3] }).then((e) => setTimeout(() => { e.delete(); }, 20000));
          kanal1.send({embeds:[shiva], components: [row2] })
        }
     }
     ...
     if (button.customId === "lgbt") {
        ...
        if(!uye.roles.cache.has(ayar.lgbt)) {
          member.setNickname(`${name} | ${age}`)
        member.roles.remove(ayar.kayıtsız)
        msg.edit({embeds: [shiva3], components: [row2] });
        if(ayar.chat && client.channels.cache.has(ayar.chat)) client.channels.cache.get(ayar.chat).send({ content:`Aramıza hoşgeldin **${uye}**! Kuralları okumayı unutma!`, components: [row3] }).then((e) => setTimeout(() => { e.delete(); }, 20000));
        kanal1.send({ embeds: [shiva3], components: [row2] })
        }
     }

     if (button.customId === "iptal") {
        if(msg) msg.delete();
        button.reply({ content:`> Bu bot shiva tarafından yapılmıştır`, embeds: [], components: [], ephemeral: true});
     }
