...
if (!uye) return message.reply({ content:` Örnek; .perm @shiva/ID`});
if(message.author.id === uye.id) return message.reply({content: `Kendine Rol Veremezsin dostum!`, ephemeral: true })

const button1 = new ButtonBuilder().setCustomId('vip').setLabel("Royale Roles").setStyle(2)
const button2 = new ButtonBuilder().setCustomId('register').setLabel("Bot Command Perm").setStyle(2)
